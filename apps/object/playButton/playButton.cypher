...
/*
This query consists of Pathfinding and Traffic Injection processes.
Contents:
1. PATHFINDING
2. INJECTION
*/
...
CALL {
  ...
  CALL {
    ...
    , [z in nodes(p) WHERE z:VPN | z.nameReal] AS VPN
    , [z in nodes(p) WHERE z:TRANSIT | z.nameReal] AS TRANSIT
...
CALL {
  ...
  CALL {
    ...
    WITH from1, to1, a, p, 
      [z in nodes(p) | z][0..size([z in nodes(p) | z])-1] AS listSize
    WHERE none(z in (listSize) WHERE z:CGW or z:CDN or z:VPN or z:TRANSIT)
    WITH distinct from1, to1, a, p,
      [z in nodes(p)| z] AS nodes,
      [z in relationships(p)| z] AS rels,
      [z in nodes(p)| z.nameReal] AS nodesName,
...
// DL DEST2, FAST QUERY, DIRECTED () - FIRST
CALL {
  MATCH (a:TRAFFIC_DOWN)
  WITH a
  CALL {
